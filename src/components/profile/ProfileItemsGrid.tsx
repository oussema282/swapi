...
export function ProfileItemsGrid({ items, isOwnProfile = true, ownerInfo }: ProfileItemsGridProps) {
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-1">
        {displayItems.map((item, index) => (
          <motion.button
            ...
            className={cn(
              'relative aspect-square overflow-hidden bg-muted group cursor-pointer'
            )}
          >
      ...
      {/* Item Details Sheet for other user's items */}
      {!isOwnProfile && (
        <>
          <ItemDetailsSheet
            open={sheetOpen}
          ...
          {selectedItem && (
            <DealInviteButton
              ...
              open={dealInviteOpen}
              onOpenChange={setDealInviteOpen}
              hideButton
            />
          )}
        </>
