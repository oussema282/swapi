...
export function DealInvitesNotification() {
  ...
  const { data: pendingInvites = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const allItemIds = [...new Set([...typedInvites.map(i => i.sender_item_id), ...typedInvites.map(i => i.receiver_item_id)])];
      const { data: items } = await supabase.from('items').select('*').in('id', allItemIds);
      const senderUserIds = [...new Set(items?.filter(i => typedInvites.some(inv => inv.sender_item_id === i.id)).map(i => i.user_id) || [])];
      const { data: profiles } = await supabase.from('profiles').select('user_id, display_name').in('user_id', senderUserIds);
      ...
      return typedInvites.map(invite => {
        ...
        return { ...invite, sender_item: senderItem ? { ...senderItem, owner_display_name: ownerProfile?.display_name || 'User' } : undefined, receiver_item: receiverItem };
  ...
  const respondMutation = useMutation({
    mutationFn: async ({ inviteId, accept }: { inviteId: string; accept: boolean }) => {
      const { error } = await supabase.from('deal_invites' as any).update({ status: accept ? 'accepted' : 'rejected', responded_at: new Date().toISOString() }).eq('id', inviteId);
      if (error) throw error;
    },
    onSuccess: (_, { accept }) => {
      if (accept) {
        toast.success(t('dealInvite.dealAccepted'));
        queryClient.invalidateQueries({ queryKey: ['matches'] });
      } else {
        toast.info(t('dealInvite.dealDeclined'));
      }
      queryClient.invalidateQueries({ queryKey: ['pending-deal-invites'] });
      queryClient.invalidateQueries({ queryKey: ['deal-invites'] });
    },
    onError: () => {
      toast.error(t('dealInvite.failedRespond'));
    },
  });
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          ...
          {pendingCount > 0 && (
            <Badge className="absolute -top-1 -right-1 w-5 h-5 p-0 flex items-center justify-center text-xs">{pendingCount}</Badge>
          )}
        </Button>
      </SheetTrigger>
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{t('matches.dealInvites')}</SheetTitle>
        </SheetHeader>
