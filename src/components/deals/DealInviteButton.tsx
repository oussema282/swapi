...
export const DealInviteButton = forwardRef<HTMLDivElement, DealInviteButtonProps>(
  function DealInviteButton({ targetItemId, targetItemTitle, className, iconOnly, open, onOpenChange, hideButton }, ref) {
    ...
    const { data: myItems = [], isLoading: itemsLoading } = useQuery({
      ...
      queryFn: async () => {
        if (!user) return [];
        const { data, error } = await supabase.from('items').select('*').eq('user_id', user.id).eq('is_active', true).eq('is_archived', false);
    ...
    const sendInviteMutation = useMutation({
      mutationFn: async (senderItemId: string) => {
        ...
        const { error } = await supabase.from('deal_invites' as any).insert({ sender_item_id: senderItemId, receiver_item_id: targetItemId });
        if (error) throw error;
        if (!isPro) await incrementUsage('deal_invites');
      },
      onSuccess: () => {
        toast.success(t('dealInvite.sent'));
      ...
      onError: (error: any) => {
        ...
        if (error.message?.includes('pending invite already exists')) {
          toast.error(t('dealInvite.pendingExists'));
        } else if (error.message?.includes('Maximum resend attempts')) {
          toast.error(t('dealInvite.maxAttempts'));
        } else if (error.message?.includes('duplicate')) {
          toast.error(t('dealInvite.alreadySent'));
        } else {
          toast.error(t('dealInvite.failedSend'));
        }
      },
    });
    ...
    const canSendInvite = (status: InviteStatus) => status === 'available' || status === 'can_resend';

    return (
      <div ref={ref}>
        {!hideButton && (
          <Button variant="outline" size={iconOnly ? 'icon' : 'sm'} className={className} onClick={(e) => { e.stopPropagation(); setShowModal(true); }}>
            <Send className="w-4 h-4" />
            {!iconOnly && <span className="ml-1.5">{t('dealInvite.inviteDeal')}</span>}
          </Button>
        )}
        ...
        <UpgradePrompt open={showUpgradePrompt} onOpenChange={setShowUpgradePrompt} feature="deal invites" featureType="deal_invites" usedCount={usage.dealInvites} limit={FREE_LIMITS.dealInvites} />
      </div>
    );
  }
);
